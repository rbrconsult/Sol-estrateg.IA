...
import { useGoogleAds, sumField, groupByDate, formatCurrency, formatNumber, formatPct } from '@/hooks/useCampaignObs';
...
export default function GoogleAdsPage() {
  ...
  const dailyChart = useMemo(() => {
    ...
    return Object.entries(byDate)
      .map(([date, dayRows]) => ({
        date,
        cost: sumField(dayRows, 'cost'),
        conversions: sumField(dayRows, 'conversions'),
      }))
  ...
  const drillDown = useMemo(() => {
    ...
    for (const r of filtered) {
      const key = `${r.campaign_name}||${r.ad_group_name || ''}||${r.dispositivo || ''}||${r.rede || ''}`;
      if (!map.has(key)) {
        map.set(key, { campaign: r.campaign_name, adGroup: r.ad_group_name, dispositivo: r.dispositivo, rede: r.rede, cost: 0, impressions: 0, clicks: 0, conversions: 0, convValue: 0 });
      }
      const m = map.get(key);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <KPICard label="Conversões" value={formatNumber(kpis.conversions)} detail={`CPA: ${formatCurrency(kpis.cpa)} | ROAS: ${kpis.roas.toFixed(2)}×`} icon={<Target className="h-4 w-4" />} />
      </div>
      ...
      <Card className="p-4">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={dailyChart}>
            ...
            <Tooltip />
            <Legend />
            <Line yAxisId="left" type="monotone" dataKey="cost" name="Custo (R$)" stroke="hsl(220, 80%, 55%)" strokeWidth={2} dot={false} />
            <Line yAxisId="right" type="monotone" dataKey="conversions" name="Conversões" stroke="hsl(var(--primary))" strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </Card>
            ...
            <TableBody>
              {drillDown.slice(0, 50).map((row, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell className="text-xs">{row.rede || '—'}</TableCell>
