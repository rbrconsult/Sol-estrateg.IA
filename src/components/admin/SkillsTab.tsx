...
const EDGE_FUNCTIONS: EdgeItem[] = [
  ...
  },
  {
    ...
    description: 'Busca logs de execução dos cenários Make para monitoramento de erros.',
    category: 'data',
    status: 'active',
    dependencies: ['MAKE_API_KEY', 'make_errors'],
  },
  {
  ...
  {
    name: 'cron-sync',
    description: 'Sincronização multi-tenant: busca DS do Make → upsert em leads_consolidados.',
    category: 'cron',
    status: 'active-cron',
    dependencies: ['MAKE_API_KEY', 'organizations', 'organization_configs', 'leads_consolidados', 'time_comercial'],
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 p-3 rounded-lg border border-border">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Em <strong>Configurações</strong>: URL da API, API Key e Nome da Instância preenchidos.
                  Atual: <code>app_settings</code> → evolution_api_url, evolution_api_key, evolution_instance_name
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 rounded-lg border border-border">
              <div className="rounded-full bg-green-500/10 p-2"><ArrowRight className="h-4 w-4 text-green-500" /></div>
              <div>
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg border border-border">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  <code>notify-ticket-whatsapp</code> e <code>send-whatsapp-alert</code> usam <code>getClaims</code> (⚠️ pode causar 401).
                  Recomendação: migrar para <code>getUser()</code>.
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 rounded-lg border border-border">
              <div className="rounded-full bg-cyan-500/10 p-2"><Webhook className="h-4 w-4 text-cyan-500" /></div>
              <div>
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg border border-border">
              <div className="rounded-full bg-orange-500/10 p-2"><Clock className="h-4 w-4 text-orange-500" /></div>
              <div>
                <p className="text-sm font-medium">5. Número Central</p>
                <p className="text-xs text-muted-foreground">
                  <code>central_whatsapp_number</code> em app_settings. Recebe cópia de todas as notificações de chamados.
                  Atual: 5511974426112
                </p>
              </div>
            </div>
