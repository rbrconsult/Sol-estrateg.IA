...
// ── Auto-activate inactive scenarios ──
...
async function activateScenario(
  ...
  try {
    ...
    if (res.ok) return { ok: true, detail: "Cenário ativado com sucesso" };
    return { ok: false, detail: `Ativação falhou [${res.status}]: ${body.substring(0, 300)}` };
  } catch (e) {
    return { ok: false, detail: `Erro ao ativar: ${e}` };
  }
}

async function runAutoActivate(
  record: any,
  makeHeaders: Record<string, string>,
  supabase: any,
  krolicKey: string | null,
  centralNumber: string | null
): Promise<{ activated: boolean; detail: string }> {
  const scenarioId = record.scenario_id;
  if (!scenarioId) return { activated: false, detail: "Sem scenario_id" };

  console.log(`[autofix] Tentando ativar cenário inativo "${record.scenario_name}" (ID ${scenarioId})`);

  // Notify start
  if (krolicKey && centralNumber) {
    await sendWhatsAppMessage(krolicKey, centralNumber, [
      "🤖 *AutoFix — Reativando Cenário*",
      "",
      `📋 *Fluxo:* ${record.scenario_name}`,
  ...
  const result = await activateScenario(scenarioId, makeHeaders);
  ...
  await supabase.from("sol_insights").insert({
    ...
    tipo: "autofix",
    titulo: result.ok
      ? `✅ AutoFix: "${record.scenario_name}" reativado`
      : `⚠️ AutoFix: falha ao reativar "${record.scenario_name}"`,
    descricao: result.detail,
    severidade: result.ok ? "info" : "warning",
    categoria: "operacional",
    robo: "skill-autofix-make",
    acao_sugerida: result.ok ? "Monitorar próximas execuções" : "Ativar manualmente no Make",
    dados: {
  ...
  if (krolicKey && centralNumber) {
    if (result.ok) {
      await sendWhatsAppMessage(krolicKey, centralNumber, [
        "✅ *REATIVADO — AutoFix Aplicado*",
        "",
        `📋 *Fluxo:* ${record.scenario_name}`,
        `🆔 *Scenario ID:* ${scenarioId}`,
        "",
        "O cenário foi reativado automaticamente com sucesso.",
      ...
      await sendWhatsAppMessage(krolicKey, centralNumber, [
        "⚠️ *AutoFix — Falha na Reativação*",
        "",
        `📋 *Fluxo:* ${record.scenario_name}`,
        `❌ *Detalhe:* ${result.detail.substring(0, 200)}`,
        "",
        "Não foi possível reativar automaticamente.",
  ...
  // Mark as resolved if activated
  if (result.ok) {
    await supabase.from("make_errors")
      .update({
        status: "resolved",
        resolved_at: new Date().toISOString(),
        resolution_notes: "Reativado automaticamente pelo AutoFix",
      })
      .eq("execution_id", record.execution_id);
  }

  return { activated: result.ok, detail: result.detail };
}
